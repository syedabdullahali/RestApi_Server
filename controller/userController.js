...
async function createOrder(name,email,amount,id,type,mobileNumber) {
...
const placeOrder = async (req, res) => {
  ...
  try {
      const response = await createOrder(name,email,amount,id,type,req.user.mobileNumber)
    if (response?.status!==200) {
       return res.status(400).json({ msg: 'Invalid Auth and Account', success: false })
    }
    
    if (!amount) {
      return res.status(400).json({ msg: 'Amount is required!', success: false })
    }
      const result = await Transaction.create({ user: id, transactionType: "online", transaction_purpose: type, 
        payment: (response.data.order_amount), order_id: response.data.order_id,cf_order_id:response.data.cf_order_id,
        payment_session_id:response.data.payment_session_id
      })
      if (result) {
        return res.status(200).json({ message: 'Order created successfully.', success: true, result: result })
      }
...
const walletAdd = async (req, res) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      const checkTransaction = await Transaction.findOne({ order_id: order_id }).session(session);
